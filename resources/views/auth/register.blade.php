            ...
            <form class="space-y-6" method="POST" action="{{ route('register') }}">
                ...
                <div>
                    <div class="flex items-center">
                        ...
                        <x-help-icon-blade class="ml-2">
                            Your username must be unique and contain no spaces
                        </x-help-icon-blade>
                    </div>
                    <div class="mt-1 relative">
                ...
                <!-- Librarian Checkbox -->
                <div class="flex items-center">
                    <input id="register_as_librarian" name="register_as_librarian" type="checkbox"
                        class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                    <label for="register_as_librarian" class="ml-2 block text-sm text-gray-900">
                        Register as Librarian
                    </label>
                </div>

                <!-- Validation Code (Hidden by default) -->
                <div id="validation-code-section" class="hidden">
                    <div class="flex items-center">
                        <label for="validation_code" class="block text-sm font-medium text-gray-700">Validation Code</label>
                        <x-help-icon-blade class="ml-2">
                            You have to request the validation code from the admin
                        </x-help-icon-blade>
                    </div>
                    <div class="mt-1">
                        <input id="validation_code" name="validation_code" type="text"
                            class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                </div>
...
<script>
    document.getElementById('register_as_librarian').addEventListener('change', function() {
        const codeSection = document.getElementById('validation-code-section');
        if (this.checked) {
